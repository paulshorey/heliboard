...
- **punctuation**: All marks permitted (`permitted_marks: ["all"]`, which includes commas, periods, `?`, `!` for English, plus locale-specific marks like `、` for Japanese). Sensitivity defaults to 0.40 — below Speechmatics' own default of 0.5, biased away from premature sentence-ending punctuation while still allowing commas.
...
- **max_delay**: Defaults to 4.0 seconds (the UI's maximum) so Speechmatics has more right-context before finalizing realtime spans. Shorter delays are more responsive but can finalize a slight pause as a separate sentence too early.
- **disfluency removal**: Optional removal of English hesitation words (um, uh, hmm)

HeliBoard rebuilds finalized text from Speechmatics token results so word spacing, punctuation attachment, and segment timing survive chunk boundaries, then commits the finalized text exactly once at the current insertion point. If the editor currently has selected text, that selection is overwritten via `commitText(...)`, matching normal typing behavior. When consecutive finalized spans are separated by less than about 1.2 seconds, `LatinIME` rewrites a trailing period from the previous span to a comma before committing the next span; this compensates for premature Speechmatics sentence breaks without affecting longer pauses, abbreviations, or explicit `!`/`?`.

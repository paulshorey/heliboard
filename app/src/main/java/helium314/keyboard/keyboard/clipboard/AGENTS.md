...
- Panel height uses `ResourceUtils.getKeyboardLayoutHeightForPanel()`; the history list is remeasured as a `layout_weight=1` child above a parser-sized, persistent `LayoutType.CLIPBOARD_BOTTOM` row (default `clip_bottom_row_with_action`). The pinned Secondary Toolbar remains available below the clipboard strip. Overlay panels do not use `fitsSystemWindows`.

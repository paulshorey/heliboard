...
- Emoji palette pages come from `assets/emoji/*.txt`; the `EMOJI_BOTTOM` and `CLIPBOARD_BOTTOM` layout slots are separate bottom-row layouts under `assets/layouts/`. Defaults are the `*_with_action` variants so enter sits on the same edge as alphabet/symbols. `KeyboardParser` shrinks those 1-row keyboards to one typing-row slot plus full bottom padding instead of dividing occupied height by 5.

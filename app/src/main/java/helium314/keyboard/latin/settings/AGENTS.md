...
- Toolbar prefs are mode-sensitive. `SettingsValues` derives `mSuggestionStripHiddenPerUserSettings`, `mSecondaryStripVisible`, `mHasPinnedToolbarKeys`, `mAutoShowToolbar`, `mAutoHideToolbar`, and `mQuickPinToolbarKeys` from `PREF_TOOLBAR_MODE` plus `PREF_TOOLBAR_KEYS`, `PREF_PINNED_TOOLBAR_KEYS`, `PREF_CLIPBOARD_TOOLBAR_KEYS`, and related flags.

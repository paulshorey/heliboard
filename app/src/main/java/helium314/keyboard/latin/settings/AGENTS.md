...
- `InputAttributes.mShouldShowSuggestions` is true for non-password text even when the host set `TYPE_TEXT_FLAG_NO_SUGGESTIONS`. `PREF_ALWAYS_SHOW_SUGGESTIONS` remains as a historical override; password and non-text fields still never look up suggestions.

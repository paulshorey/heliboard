...
- Soniox prefs: `PREF_SONIOX_API_KEY`, endpoint detection, max endpoint delay (default 3000 ms; `LEGACY_PREF_SONIOX_MAX_ENDPOINT_DELAY_MS` = 2000 for upgrade migration), diarization, custom `context.terms` (`PREF_SONIOX_CUSTOM_TERMS`), and `PREF_SONIOX_REMOVE_COMMAS` (default on; strips commas from finalized transcript text after other post-processing). `TranscriptionPreferences` clears legacy Speechmatics/Deepgram API-key prefs on first read.

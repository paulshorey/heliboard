...
- Local voice prefs (`PREF_VOICE_CHUNK_SILENCE_SECONDS`, `PREF_VOICE_SILENCE_THRESHOLD`, `PREF_VOICE_AUTO_STOP_SILENCE_SECONDS`) feed `VoiceRecorder`: chunk silence sends Gemini `audioStreamEnd` from `VoiceInputManager` and then holds outbound audio until speech resumes, while auto-stop silence stops recording after a longer pause. Keep the chunk-silence default (2 s) longer than `PREF_GEMINI_END_OF_SPEECH_SILENCE_MS` so the server's semantic end-of-speech detection normally decides where a sentence ends. `TranscriptionPreferences.migrateGeminiVoiceDefaults` rewrites a persisted old default of 1 s to 2 s once. A leftover interim is flushed if no final arrives after `audioStreamEnd`.
...
- `PREF_SHOW_PINNED_TOOLBAR` (default on) is the user toggle for the second toolbar row of pinned keys. `SettingsValues.mShowPinnedToolbar` snapshots it; `SuggestionStripView` hides `secondary_toolbar_container` when it is off so IME height/insets drop that row.

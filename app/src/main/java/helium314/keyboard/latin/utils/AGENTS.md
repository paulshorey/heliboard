...
- `ResourceUtils.java` - resource lookup helpers (keyboard width/height; note `getDefaultKeyboardHeight` clamps `config_default_keyboard_height` against `config_min/max_keyboard_height`, where a negative min is interpreted as a fraction of **display width**). `getKeyboardLayoutHeightForPanel` returns only the overlay body height; primary/pinned toolbar strips are sibling chrome and are never folded into that value.
...
- Pinned secondary-toolbar keys must use `applyPinnedToolbarKeyLayout` (equal `LinearLayout` weights, zero minWidth, cleared horizontal padding) so they fit `secondary_toolbar.xml` without overflow. The expandable toolbar in `suggestions_strip.xml` still uses `HorizontalScrollView`; only the pinned strip is even-width.
- Toolbar `EMOJI` sends `KeyCode.EMOJI`; toggle behavior lives in `keyboard/internal/KeyboardState.kt`.

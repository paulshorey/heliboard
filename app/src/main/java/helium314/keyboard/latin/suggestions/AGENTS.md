...
- Toolbar `EMOJI` sends `KeyCode.EMOJI`. `KeyboardState` toggles that code: the first press shows the emoji palette, and a second press while it is showing returns to the alphabet keyboard.

...
- Pinned toolbar keys render as an equal-width row in the **Secondary Toolbar** (`R.id.pinned_keys` in `secondary_toolbar.xml`), a sibling strip below `SuggestionStripView` in `main_keyboard_frame.xml`. `KeyboardSwitcher` calls `populatePinnedKeys()` after inflation; each button uses `applyPinnedToolbarKeyLayout` from `ToolbarUtils.kt`. `LatinIME` includes secondary-toolbar height in strip/inset math. The same host contains an emoji-only search prototype which replaces the pinned-key row while emoji mode is visible.
- Toolbar `EMOJI` sends `KeyCode.EMOJI`. `KeyboardState` toggles from the visible palette (`SwitchActions.isShowingEmojiKeyboard`): the first press shows emoji, and a second press while it is showing returns to alphabet. Physical shortcuts can change the UI without updating `KeyboardState.mode`.

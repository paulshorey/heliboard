...
- Pinned toolbar keys render as an equal-width row in the **Secondary Toolbar** (`R.id.pinned_keys`), a sibling strip below `SuggestionStripView` in `main_keyboard_frame.xml`; `KeyboardSwitcher` calls `populatePinnedKeys()` after full inflation, and `LatinIME` includes that height in insets. The same host contains an emoji-only search prototype which replaces the pinned-key row while emoji mode is visible.

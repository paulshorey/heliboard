...
- Pinned toolbar keys render in the **Secondary Toolbar** (`R.id.pinned_keys`), a sibling strip below `SuggestionStripView` in `main_keyboard_frame.xml`; `KeyboardSwitcher` calls `populatePinnedKeys()` after full inflation, and `LatinIME` includes that height in insets. The row is a full-width `LinearLayout` (not a scroll view): every pinned key gets equal weight via `applyPinnedToolbarKeyLayout`, and 1dp start/end padding matches the first-row more-key and microphone edge inset.

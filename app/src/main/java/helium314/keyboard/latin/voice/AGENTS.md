...
- HeliBoard only commits `is_final` tokens. Tokens become final via (a) server endpoint detection, (b) manual finalize, or (c) end-of-stream. `max_endpoint_delay_ms` is only the **maximum** wait — Soniox's semantic endpoint can fire much sooner (premature sentence end) or, when the model is unsure, be delayed so long the trailing phrase never commits. To bound that, `VoiceInputManager` sends a manual finalize (`SonioxTranscriptionClient.finalizeNow()`, control frame `{"type":"finalize"}`) when the local `VoiceRecorder` silence detector reports `onSpeechStopped`. It fires at most once per speech→silence transition (re-armed on the next `onSpeechStarted`) and is rate-limited by `MIN_MANUAL_FINALIZE_INTERVAL_MS`. This guarantees the tail is committed at the user's chunk-silence pause and lets endpoint detection be turned off entirely (finalize then runs purely off local VAD).

...
- `FullappDraftsScreen.kt` - edit history screen (merged live fullapp drafts + bounded regular/fullapp history, retention slider, enable toggle).
...
- `ToolbarScreen.kt` is mode-conditional: settings for pinned keys, auto show/hide, and quick pin only matter in the `ToolbarMode`s where `SettingsValues` enables those behaviors. `PREF_SHOW_PINNED_TOOLBAR` appears with the pinned-keys picker in `EXPANDABLE` and `SUGGESTION_STRIP` modes (default on). Toggling it reloads the IME so the second row and window height update together.

...
- `clipboard_bottom/*.json` and `emoji_bottom/*.json` - bottom rows with and without action-key variants. Defaults are `emoji_bottom_row_with_action` and `clip_bottom_row_with_action`; the 3-key built-in variants remain selectable.

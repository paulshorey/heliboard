...
4. **While editing**: Fullscreen text is continuously persisted as `fullscreen_in_progress` in the shared session store (debounced + lifecycle flush), with redundant selection-only/no-op updates skipped to keep timestamps meaningful. If text briefly changes and then reverts before debounce fires, the pending write is canceled.
   - Entering fullscreen always forces/normalizes session state to `fullscreen_in_progress` even if text is unchanged from a previous `pending_sync` snapshot.
...
8. **Regular flush safety**: lifecycle regular-mode snapshot flushes are gated by actual content edits in the current field, reducing accidental overwrite of pending sessions when focus/visibility changes without user edits.

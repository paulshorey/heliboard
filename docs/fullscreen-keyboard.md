...
- If the current editor has only a weak key (package-only), session lookup may recover the latest same-package session as a pragmatic fallback for environments where field identity is unavailable.
